...
module.exports.ytAuthorIconGet = async (authorId, resize) => {
    ...
    if (!fs.existsSync(savePass + "cache/YouTubeAuthorIcon/" + authorId + ".jpg")) { //画像が無かったら
        //各大きさのサムネイルリストを取得
        ...
        await new Promise(async resolve => {
            ...
            axios.get(thumbnails[thumbnails.length - 1].url, { responseType: "arraybuffer" })
                .then(res => {
                    fs.writeFileSync(savePass + "cache/YouTubeAuthorIcon/" + authorId + ".jpg", new Buffer.from(res.data), "binary") //保存
    ...
    if (resize && !fs.existsSync(savePass + "cache/YouTubeAuthorIconRatioResize/" + authorId + "-r" + resize.ratio + "-" + resize.size + ".jpg")) {
        if (!fs.existsSync(savePass + "cache/YouTubeAuthorIcon/" + authorId + ".jpg")) return
        await new Promise(async resolve => {
            //大きさなどを取得
            const { width, height, type } = await imageSize(savePass + "cache/YouTubeAuthorIcon/" + authorId + ".jpg")
            ...
            //キャッシュから画像を取得する
            const Stream = fs.createWriteStream(savePass + "cache/YouTubeAuthorIconRatioResize/" + authorId + "-r" + resize.ratio + "-" + resize.size + ".jpg")
            sharp(savePass + "cache/YouTubeAuthorIcon/" + authorId + ".jpg").resize(aspx * y, aspy * y).pipe(Stream).on("error", e => {
