...
## コンソール画面を見るには
あなたのブラウザはEdge、Chrome、Firefox、Safariの４択だと思われます。それぞれ、Hello Worldを見る、つまりコンソール画面を見る方法を教えます。

では今からプラットフォーム順に方法を紹介します。基本的に殆ど同じ方法で開くことが出来ます。
### Edgeの場合  
<img src="image-11.png" width="25%"><br>
直接コンソールを開くには「Ctrl+Shift+J」を同時に押します。すぐにコンソール画面が出来るため、既に「Hello World」を見つけることが出来ているはずです。

または、キーボードショートカット「Ctrl+Shift+C」で「開発者ツール」というものが開きます。そこの下側に「コンソール」というタブがあるので、そこを見るか、上側に「要素」それの横に「>>」があると思うので、それをクリックし、一覧から「コンソール」をクリックします。すると「Hello World」を見つけることが出来ます。
...
キーボードショートカット「Ctrl+Shift+C」で「DevTools」というものが開きます。上側に「Switch DevTools to Japanese」というボタンがあれば、押して置くと役立ちます。その後再起動がかかることがあるので、少し待ったら、上に「コンソール」というボタンがあるので、クリックすることでHello Worldを見つけることが出来ます。
...
### 「;」セミコロン
次はよくプログラムの最後についていた「;」について説明します。
「;」は**セミコロン**といい、1つの**プログラムの終わり**として使います。
これは例です。様々なわからない例が出てきますが、後々振り返るときに役に立つので、貼っておきます。
```js
// 基本
const string = "文字列"; // 何かを定義または初期化するときに使う
let number = 10;
// 常用は推奨はしないが、一部のコードで見栄えを上げるために並列化させるために使うことが出来る
const string2 = "文字列2"; let number2 = 200;

// 後半解説
number + 10; // 計算後に設置
func(); // 関数の呼び出しをした最後に設置
import fs from "fs"; // モジュールを呼び出した後に設置
