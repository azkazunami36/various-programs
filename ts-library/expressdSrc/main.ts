...
addEventListener("load", async () => {
    ...
    }
    {
        ...
        sendButton.addEventListener("click", async () => {
            resText.innerText = await API.testAPI.one("テスト")
        })
        apiTest.appendChild(sendButton)
        apiTest.appendChild(resText)
        shareData.windowSystem.createWindow("testAPI", apiTest, new createWindowOptionBuilder()
            .setTitle("APIテスト")
        )
    }
})
